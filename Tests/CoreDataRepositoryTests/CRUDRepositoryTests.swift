...
final class CRUDRepositoryTests: CoreDataXCTestCase {
    ...
    func testReadSuccess() async throws {
        ...
        let result: Result<Movie, CoreDataRepositoryError> = try await repository().read(try XCTUnwrap(createdMovie.url))
    ...
    func testReadFailure() async throws {
        ...
        _ = try await repositoryContext().perform {
            let objectID = try self.repositoryContext().persistentStoreCoordinator?.managedObjectID(forURIRepresentation: try XCTUnwrap(createdMovie.url))
    ...
    func testUpdateSuccess() async throws {
        ...
        let result: Result<Movie, CoreDataRepositoryError> = try await repository().update(try XCTUnwrap(createdMovie.url), with: movie)
    ...
    func testDeleteSuccess() async throws {
        ...
        let result: Result<Void, CoreDataRepositoryError> = try await repository().delete(try XCTUnwrap(createdMovie.url))
    ...
    func testReadSubscriptionSuccess() async throws {
        ...
        let count: Int = try await repositoryContext().perform { [self] in
            return try self.repositoryContext().count(for: RepoMovie.fetchRequest())
        }
